...
void							add_or_update_env(t_shell *shell, char *key,
									char *value);
...
t_pipex							*append_pipe_node(t_pipex *pipex,
									t_pipex *current);
void							parse_env_var(t_env *env);
void							add_env_to_shell(t_shell *shell,
									char *env_line);
t_cmd							*create_cmd_node(t_shell *shell, char **args);
void							add_cmd_to_shell(t_shell *shell,
									t_cmd *new_cmd);
char							*get_env_value(char *name, char **envp);
void							ft_print_env_path(char **envp);
char							*append_char(char *str, char c);
char							*get_env_value_from_list(t_env *env,
									char *name);
char							*expand_vars(char *str, t_env *env);
void							main_cleaner(t_shell *shell);
void							prompt_loop(t_shell *shell);
void							exec_direction(t_shell *shell, char *line);
char							*expand_variable_from_env(char *str, t_env *env,
									char *res, int *i);
char							*expand_dollar(char *str, t_env *env, char *res,
									int *i);
...
bool							handle_special_loop(t_token **begin,
									t_token_type spe, int *i);
...
void							token_add_front(t_token **first,
									t_token *new_token);
bool							is_space(char c);
bool							handle_special_case(t_token **begin,
									char *symbol, t_token_type type);
t_token_type					is_special(const char *str);
bool							add_special(t_token **begin, char **cmd);

// /* Initialisation de la struct */
t_pipex							*init_minishell_pipeline(t_cmd *cmd_list,
									t_shell *shell);
